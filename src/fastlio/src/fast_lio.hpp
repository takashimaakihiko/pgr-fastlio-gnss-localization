...
{
public:
    FastLio(const std::string &config_path, const std::string &output_dir);
    ~FastLio();
...
FastLio::FastLio(const std::string &config_path, const std::string &output_dir) : odom_result(new custom_messages::Odometry), output_file(output_dir + "output.txt"), exec_time_file(output_dir + "time.txt")
{
...
bool FastLio::has_pose()
{
    return odom_result->header.stamp.toSec() > 0.0;
}

double FastLio::get_timestamp()
{
    return odom_result->header.stamp.toSec();
}
